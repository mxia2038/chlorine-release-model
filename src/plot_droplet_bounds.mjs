...
const metrics=[
  ['fanNm3H','Fan flow'],
  ['maxGasMolFraction','Gas Cl₂ peak'],
  ['maxTotalChlorineKgH','Total Cl₂ peak'],
  ['mainCoolingKW','Cooling peak']
];
const colors=['#2b6cb0','#c05621','#2f855a'];
const labels=['Suspended','Vaporized','Deposited to pool'];
const ratios=rows.map(r=>metrics.map(([k])=>r[k]/rows[0][k]));
const W=900,H=430,left=70,top=55,plotW=790,plotH=300,maxY=1.6;
...
body+=`<text x="${W/2}" y="25" text-anchor="middle" class="title">Droplet-fate boundary comparison</text>`;
